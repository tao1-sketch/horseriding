(function () {
    ...
    const BASE_MIN_DURATION = 29000;
    const BASE_MAX_DURATION = 43000;
    ...
    const ellipseScaleX = 12.0 + Math.random() * 3.5;
    const ellipseScaleZ = 6.0 + Math.random() * 2;
    ...
    // test1_안되면 삭제
    let lastRaceUpdateTime = 0;
    let predictedRemainingSec = null;
    let finalPhaseActivated = false;
    ...
    function initThree() { // 화면 크기
        ...
        camera.lookAt(0, 0, 0); // 기존 크기

        /*const targetAspect = 16 / 9;
        const minHeight = 600;
        const idealHeight = width / targetAspect;
        if (height < idealHeight) height = idealHeight;
        if (height < minHeight) height = minHeight;

        width = Math.floor(width);
        height = Math.floor(height); // 변경식*/
    ...
    function onResize() {
        ...
        /*const targetAspect = 16 / 9; // 화면 크기 조절
        const minHeight = 480;
        const idealHeight = width / targetAspect;
        if (height < idealHeight) height = idealHeight;
        if (height < minHeight) height = minHeight;
        ...
        width = Math.floor(width);
        height = Math.floor(height); //추가된 부분*/
    ...
    function updateLegendOrder() {
        ...
        sorted.forEach((h, idx) => {
            ...
            if (!el) return;
            /*if (h.legendSpeedEl && typeof h.debugSpeed === "number") {
                h.legendSpeedEl.textContent = ` ×${h.debugSpeed.toFixed(3)}`;
            }*/
            if (h.legendSpeedEl) {
                h.legendSpeedEl.textContent = "";
            }
            if (idx === 0) {
    ...
    function setupRace() {
        ...
        // test2_안되면 삭제
        lastRaceUpdateTime = 0;
        predictedRemainingSec = null;
        finalPhaseActivated = false;
        finalPhaseInitialDir = null;
        ...
        names.forEach((name, idx) => {
            ...
            speedSpan.className = "legend-speed";
            //speedSpan.textContent = ""; // 속도 표시 필요할 때 display랑 동시에 활성화
            speedSpan.style.display = "none";

            item.appendChild(colorBox);
            item.appendChild(nameSpan);
            item.appendChild(speedSpan);
            ...
            horses.push({
                ...
                innerLaneBias: 0,
                legendItem: item,
                legendNameEl: nameSpan,
                legendSpeedEl: speedSpan,
                debugSpeed: 1
            });
    ...
    function updateRace(time) {
        ...
        //test4_lastRace...까지
        let dtSec = 0.016;
        if (lastRaceUpdateTime > 0) {
            const dt = time - lastRaceUpdateTime;
            if (dt > 0 && dt < 200) {
                dtSec = dt / 1000;
            }
        }
        lastRaceUpdateTime = time;
        ...
        if (globalT < 0.7 && activeContestIndices.length > 1) {
        ...
        if (n > 1) {
            ...
            /*if (progPairs.length) {
                leaderPrevProgress = progPairs[0].prog || 0;
            }*/
        ...
        horses.forEach((h, idx) => {
            ...
            if (useContestMean && isContestHorse[idx]) {
                ...
                contestPull = clamp(diff * 2.0 * intensity, -0.45 * intensity, 0.45 * intensity);

                const tLocal = time * 0.001 + phases[idx] * 0.7;
                contestWiggle = Math.sin(tLocal * 4) * 0.08 * intensity;
            }
            speedDebug.contest = contestPull + contestWiggle;
            ...
            const cornerSpeedBonus = 0.7 + cornerAmountSpeed * innerFactor * 0.35;
            speedDebug.laneCorner = cornerSpeedBonus - 1;
            ...
            speedDebug.rank = midBoost + finalBoost;

            let noiseFactor = 1 + speedDebug.noise;
            let contestFactor = 1 + speedDebug.contest;
            let laneFactor = 1 + speedDebug.laneCorner;
            let rankFactor = 1 + speedDebug.rank;
            ...
            if (typeof h.lastSpeedMult === "number") {
                const SMOOTH = 0.35;
                speedMult = h.lastSpeedMult + (speedMult - h.lastSpeedMult) * SMOOTH;
            }
            h.lastSpeedMult = speedMult;

            if (speedMult < 0.15) speedMult = 0.15;
            h.debugSpeed = speedMult;
            h.debugSpeedComponents = speedDebug;
            ...
            horses.forEach((h, idx) => {
                ...
                const maxDelta = maxSpeed * dtSec; // 프레임 당 최대 이동량(좌우)
            ...
            if (!raceFinished && n > 0) {
                const leadIdx = leaderIndex;
                const prevP = prevProgresses[leadIdx] || 0;
                const curP  = newProgresses[leadIdx] || 0;
                const dP    = curP - prevP;

                if (dtSec > 0 && dP > 0.000001) {
                    let est = (1 - curP) / (dP / dtSec);
                    if (isFinite(est) && est >= 0) {
                        est = clamp(est, 0, 30);
                        if (typeof predictedRemainingSec === "number") {
                            const alpha = 0.25;
                            predictedRemainingSec =
                                predictedRemainingSec + (est - predictedRemainingSec) * alpha;
                        } else {
                            predictedRemainingSec = est;
                        }
                    }
                }
            } // test5_안될시 함수 전체 삭제
            ...
            function updateCamera(time) {
                if (introActive && horses.length) {
                    ...
                    const distAhead = Math.max(4.0, spread * 2.4);
                    const height = Math.max(3.5, spread * 0.7);
                ...
                if (countdownActive && horses.length) {
                    ...
                    const spread = trackWidth || 3;
                    const distAhead = Math.max(ellipseScaleX, spread * 3.2);
                    const height = Math.max(6.0, spread * 1.0);
                ...
                if (running && horses.length) {
                    ...
                    const leaderProgress = leader ? (leader.progress || 0) : 0;

                    // test6_안되면 주석 전까지 삭제후 주석 해제
                    const remainingSecEst =
                    (typeof predictedRemainingSec === "number") ? predictedRemainingSec : Infinity;
                    if (!finalPhaseActivated) {
                        const inFinalTime      = remainingSecEst > 0 && remainingSecEst <= 5;
                        const inFinalProgress  = leaderProgress >= 0.78; // 너무 초반에 켜지는 것 방지

                        if (inFinalTime && inFinalProgress) {
                            finalPhaseActivated = true;
                        }
                    } else {
                        const sec = Math.max(0,Math.min(5,(typeof predictedRemainingSec === "number" ? predictedRemainingSec : 0)));
                        finalPhaseCamera(sec);
                        return;
                    }

                    /*const remaining = total - elapsed;
                    let remainingSec = remaining / 1000;
                    if (!isFinite(remainingSec)) remainingSec = 0;

                    const inFinalTime = remainingSec > 0 && remainingSec <= 5;
                    const inFinalProgress = leaderProgress >= 0.78;
                    if (inFinalTime && inFinalProgress) {
                        finalPhaseCamera(remainingSec);
                        return;
                    }*/
